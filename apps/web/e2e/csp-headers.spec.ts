...
test.describe('CSP Headers', () => {
  ...
  test('should have different CSP headers in production mode', async ({ page }) => {
    ...
    const reportOnlyHeader = Object.entries(headers).find(
      ([key]) => key.toLowerCase() === 'content-security-policy-report-only'
    )?.[1];
    const strictHeader = Object.entries(headers).find(
  ...
  test('should not apply CSP to static assets', async ({ page }) => {
    ...
    const staticPaths = ['/_next/static/chunks/webpack.js', '/favicon.ico'];
