...
```bash
# 1. Run linting to fix auto-fixable issues
pnpm lint
...
1. **Remove unused imports immediately** - ESLint will fail on any unused import
2. **Fix TypeScript errors before committing** - Build must succeed
3. **Run lint locally before push** - CI will fail if lint fails
4. **Use consistent formatting** - Prettier should handle this automatically
5. **Follow zero-warnings policy** - Any warnings will cause CI failure
...
- **If pre-commit hooks block your commit**:
  ...
  # Option 2: Bypass hooks (ONLY for emergencies)
  git commit --no-verify -m "your message"
  ```

- **CI Pipeline Requirements**:
  - All lint checks must pass (`pnpm lint`)
  - All builds must succeed (`pnpm build`)
